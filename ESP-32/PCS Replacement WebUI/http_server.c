...
const char html[] = "<!DOCTYPE html><html><head><meta name=\"viewport\" content=\"width=device.width, initial-scale=1\"><title>Plunger Radio</title><meta http-equiv=\"refresh\" content=\"1\"></head><body><h1 style=\"text-align: center;\">Radio Web View</h1><div style=\"text-align: center;\"><p id=\"solenoidstate\">Sales Solenoid(0=OFF 1=ON): </p><p id=\"plungerstate\">Plunger State (0=ON 1=OFF): </p><p id=\"meterstate\">Meter Sales Output(0=OFF 1=ON): </p><p id=\"relaystate\">Meter Plunger Input(0=OFF 1=ON): </p></div><script>function fetchVariables() {fetch('/variables').then((response) => response.json()).then((data) => {var solenoidstate = data.solenoidstate;var plungerstate = data.plungerstate;var meterstate = data.meterstate;var relaystate = data.relaystate;document.getElementById('solenoidstate').innerHTML += solenoidstate;document.getElementById('plungerstate').innerHTML += plungerstate;document.getElementById('meterstate').innerHTML += meterstate;document.getElementById('relaystate').innerHTML += relaystate;});}window.onload = fetchVariables;</script></body></html>";
